...
tf.app.flags.DEFINE_enum('image_format', 'jpeg', ['jpg', 'jpeg', 'png'],
                         'Image format.')
...
class ImageReader(object):
  ...
  def __init__(self, image_format='jpeg', channels=3):
    ...
    with tf.Graph().as_default():
      ...
      #encoding, 20180725_encoding 부분 추가 JJ
      if self._image_format in ('jpeg', 'jpg'):
...
# 20180725_encoding 부분 추가 JJ
  def encode_png(self, img_data):
    """Decodes the image data string.
    ...
    image = self._session.run(self._encode,
                              feed_dict={self._encode_data: img_data})
